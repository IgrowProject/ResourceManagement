...
namespace IGrow.Web.Areas.Api.Controllers
{
    public class UploadController : Controller
    {
        ...
        public ActionResult upLoadfile()
        {
            ...
            string _delfile = HttpHelper.GetString("DelFilePath"); //要删除的文件
            string fileName = HttpHelper.GetString("name"); //文件名

            byte[] byteData = FileHelper.ConvertStreamToByteBuffer(Request.InputStream); //获取文件流
            bool _iswater = false; //默认不打水印
            bool _isthumbnail = false; //默认不生成缩略图

            if (HttpHelper.GetQueryString("IsWater") == "1")
            {
                _iswater = true;
            }
            if (HttpHelper.GetQueryString("IsThumbnail") == "1")
            {
                _isthumbnail = true;
            }
        ...
        public ActionResult uploadimage()
        {
            ...
            return FileSave(upFile, _iswater);
        }

        /// <summary>
        /// 上传图片
        /// </summary>
        public ActionResult uploadimagebase64(string filename,string filesrc,string filetype)
        {
            bool _iswater = true; //默认打水印
            byte[] byteData = Convert.FromBase64String(filesrc);//获取图片base64

            //开始上传
            string remsg = new UpLoad().FileSaveAs(byteData, filename, false, _iswater);
        ...
        /// 统一保存文件
        /// </summary>
        private ActionResult FileSave(HttpPostedFileBase upFiles, bool isWater)
        {
            if (upFiles == null)
            {
                return showError("请选择要上传文件！");
        ...
        /// 显示成功提示
        /// </summary>
        private ActionResult showSuccess(string fileName, string filePath)
        {
            Dictionary<string, object> dic = new Dictionary<string, object>();
            dic.Add("state", "SUCCESS");
            dic.Add("url", filePath);
            dic.Add("title", fileName);
            dic.Add("original", fileName);
            return Content( dic.ToJson());
        ...
        /// 浏览目录文件
        /// </summary>
        private ActionResult ListFileManager(string filePath, string fileTypes)
        {
            Dictionary<string, object> dic = new Dictionary<string, object>();
            int Start = HttpHelper.GetInt("start", 0); //开始索引
            int Size = HttpHelper.GetInt("size", 20); //每页大小
            int Total = 0; //文件总数
            ...
            {
                dic.Add("state", State);
                dic.Add("list", FileList == null ? null : FileList.Select(x => new { url = x }));
                dic.Add("start", Start);
                dic.Add("total", Total);
            }
            return Content(dic.ToJson());
