    ...
    public class ModuleButtonController : ControllerBase
    {
        ...
        public ActionResult GetTreeGridJson(string moduleId)
        {
            ...
            foreach (ModuleButtonEntity item in data)
            {
                ...
                bool hasChildren = data.Count(t => t.F_ParentId == item.IGrowID) == 0 ? false : true;
                treeModel.id = item.IGrowID;
        ...
        public ActionResult GetCloneButtonTreeJson()
        {
            ...
            foreach (ModuleEntity item in moduledata)
            {
                ...
                bool hasChildren = moduledata.Count(t => t.F_ParentId == item.IGrowID) == 0 ? false : true;
                tree.id = item.IGrowID;
            ...
            {
                TreeViewModel tree = new TreeViewModel();
                bool hasChildren = buttondata.Count(t => t.F_ParentId == item.IGrowID) == 0 ? false : true;
                tree.id = item.IGrowID;
                tree.text = item.F_FullName;
                tree.value = item.F_EnCode;
