...
using IGrow.Application.DingTalk;
        ...
        public  static string GetUserName(string usersid)
        {
            ...
            if (!string.IsNullOrEmpty(usersid))
            {
                ...
                foreach (string uid in users)
                {
                    ...
                    {
                        UserEntity user = userlist.Find(p => p.IGrowID == uid);
                        if (user != null)
                        {
            ...
            if (usersids.Count > 0)
            {
                ...
                foreach (string uid in usersids)
                {
                    if (!string.IsNullOrEmpty(uid))
                    {
                        ...
                        {
                            UserEntity user = userlist.Find(p => p.IGrowID == child);
                            if (user != null && !string.IsNullOrEmpty(user.F_DingTalkUserId))
                            {
                                dUseridList.Add(user.F_DingTalkUserId);
                            }
            ...
            if (usersids.Count > 0)
            {
                ...
                foreach (string uid in usersids)
                {
                    if (!string.IsNullOrEmpty(uid))
                    {
                        ...
                        if (user != null && !string.IsNullOrEmpty(user.IGrowID))
                        {
                            dUseridList.Add(user.IGrowID);
                        }
                    }
                }
                return dUseridList.Distinct().ToList(); ;
